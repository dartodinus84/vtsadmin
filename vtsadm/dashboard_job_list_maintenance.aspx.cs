...
namespace vtsadm
{
    public partial class dashboard_job_list_maintenance : System.Web.UI.Page
    {
        private const string SessionDataKey = "RecDashboardJobListMaintenance";
        ...
        protected void gvMaintenance_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            gvMaintenance.DataSource = cachedTable;
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    "JobID", "sRegDate", "CustomerName", "MarketingName", "sSchDate", "CloseDate", "sla", "Remark", "BillAbleDesc",
                    "IsMigrationDesc", "RegionalName", "AreaName", "AreaGroupName", "ValueStatus"
                };

                string[] headers = new string[]
                {
                    "Job ID", "Register Date", "Customer Name", "Marketing", "Schedule Date", "Close Date", "SLA", "Remark", "Billable",
                    "Migration", "Regional Name", "Area Name", "Area Group Name", "Status"
                };

                DataTable exportTable = BuildExportTable(sourceTable, dataFields, headers);
                string fileName = "JobMaintenance_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xls";
                string regionText = string.IsNullOrWhiteSpace(regionDisplay) ? "All" : regionDisplay;
                string dateRange = dateFromQuery + " - " + dateToQuery;

                ExportDataTableToXls(exportTable, fileName, "Job Maintenance - " + statusTitle, regionText, dateRange);
            }
            catch (Exception ex)
            {
                Response.Clear();
        ...
        private void SetHeaderContext(string status, string regionDisplay, string dateFrom, string dateTo)
        {
            ...
            lblHeaderTitle.Text = "Maintenance - " + statusTitle;
            lblContextStatus.Text = "Maintenance - " + statusTitle;
        ...
        private DataTable ExecuteDataTable(string filterType, string status, string region, string dateFrom, string dateTo)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("sp_dashboard_job_list_maintenance", conn))
